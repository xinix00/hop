...
- **Fail-safe when the leader is lost** (`internal/agentloop`): after 4 failed ticks the agent tries to take over; after 7 it stops all its tasks — but ONLY if the lock store still reports a live leader (we are the isolated one) or the store itself is unreachable (cannot tell). If the store answers "no leader", tasks keep running: nobody re-places them, stopping would be pure loss. Store calls run off the tick (`AsyncDiscoverer`); the lease expiry is the leader's timer.
